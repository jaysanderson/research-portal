...
export default function EmbedPage() {
  ...
  const placeholder = profile?.subject ? `Ask about ${profile.subject}…` : 'Ask about the knowledge base…';
  const examples = profile?.exampleQuestions?.length ? profile.exampleQuestions.slice(0, 3) : [];
  return (
    <div className="flex h-screen flex-col bg-white">
      ...
      <div className="flex-1 overflow-hidden">
        <ChatThread messages={messages} busy={busy} onSend={send} onStop={stop} compact placeholder={placeholder} examples={examples} />
      </div>
    </div>
  );
}
